...
function fizzBuzz(numero) {
  ...
  var status = 0 
  if(numero % 3 === 0){status = 1;}
  if (numero % 5 === 0){status += 2;}
  if (status > 0){
    var str = ""
    if (status === 1){str = "fizz";}
    if (status === 2){str = "buzz";}
    if (status === 3){str = "fizzbuzz";}
    return str
  }else{return numero}
}
...
function operadoresLogicos(num1, num2, num3) {
  ...
  for(i=0;i < lista.length;i++){
    if (lista[i] === 0){return "Error"}
    if (lista[i] < 0){return "Hay negativos"}
  }
